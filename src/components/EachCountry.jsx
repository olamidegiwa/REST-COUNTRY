...
const EachCountry = ({ country }) => {
  ...
  const borders = country.borders
    ? country.borders.map((border) => {
        return (
          <p className="bg-elements px-3 px-4 rounded-1" key={border}>
            {border}
          </p>
        );
      })
  ...
  return (
    <div className="bg-main-color custom-text-white text-start px-5 py-5 each-country">
      <div
        ...
        className="d-flex gap-3 mb-5 bg-elements back-btn align-items-center py-2 px-4 rounded-2 shadow"
      >
        <BiLeftArrowAlt className="fs-4 custom-text-white" />
